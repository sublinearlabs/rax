...
impl RegisterMapper for RegisterMapping {
        ...
        Self: Sized,
    {
        ...
        let map = [
            ...
            RegisterLocation::MEM(mem_addr + 8), // x5(t0): memory (freed R11 for scratch register)
            ...
            RegisterLocation::MEM(mem_addr + 16), // x9(s1): memory (callee-saved, freed R13 for scratch)
            ...
            RegisterLocation::MEM(mem_addr + 24), // x16(a6): memory (overflow)
            RegisterLocation::MEM(mem_addr + 32), // x17(a7): memory (overflow)
            RegisterLocation::MEM(mem_addr + 40), // x18(s2): memory (callee-saved)
            RegisterLocation::MEM(mem_addr + 48), // x19(s3): memory (callee-saved)
            RegisterLocation::MEM(mem_addr + 56), // x20(s4): memory (callee-saved)
            RegisterLocation::MEM(mem_addr + 64), // x21(s5): memory (callee-saved)
            RegisterLocation::MEM(mem_addr + 72), // x22(s6): memory (callee-saved)
            RegisterLocation::MEM(mem_addr + 80), // x23(s7): memory (callee-saved)
            RegisterLocation::MEM(mem_addr + 88), // x24(s8): memory (callee-saved)
            RegisterLocation::MEM(mem_addr + 96), // x25(s9): memory (callee-saved)
            RegisterLocation::MEM(mem_addr + 104), // x26(s10): memory (callee-saved)
            RegisterLocation::MEM(mem_addr + 112), // x27(s11): memory (callee-saved)
            RegisterLocation::MEM(mem_addr + 120), // x28(t3): memory (caller-saved temp, spilled)
            RegisterLocation::MEM(mem_addr + 128), // x29(t4): memory (freed R14 for scratch)
            RegisterLocation::MEM(mem_addr + 136), // x30(t5): memory (spilled temp)
            RegisterLocation::MEM(mem_addr + 144), // x31(t6): memory (spilled temp)
        ];
