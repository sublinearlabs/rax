...
pub(super) fn emit_instruction(
    ...
    match insn {
        ...
        Instruction::Or(R { rd, rs1, rs2 }) => {
            rv64i::emit_or(translator, temps, rv(rd), rv(rs1), rv(rs2))
        }
        ...
        Instruction::Sb(S { rs1, rs2, imm }) => {
            rv64i::emit_sb(translator, temps, rv(rs1), rv(rs2), *imm)
        }
        Instruction::Sd(S { rs1, rs2, imm }) => {
            rv64i::emit_sd(translator, temps, rv(rs1), rv(rs2), *imm)
        }
        ...
        Instruction::Lb(I { rd, rs1, imm }) => {
            rv64i::emit_lb(translator, temps, rv(rd), rv(rs1), *imm)
        }
        Instruction::Lbu(I { rd, rs1, imm }) => {
            rv64i::emit_lbu(translator, temps, rv(rd), rv(rs1), *imm)
        }
        Instruction::Lh(I { rd, rs1, imm }) => {
            rv64i::emit_lh(translator, temps, rv(rd), rv(rs1), *imm)
        }
        Instruction::Lhu(I { rd, rs1, imm }) => {
            rv64i::emit_lhu(translator, temps, rv(rd), rv(rs1), *imm)
        }
        Instruction::Lw(I { rd, rs1, imm }) => {
            rv64i::emit_lw(translator, temps, rv(rd), rv(rs1), *imm)
        }
        Instruction::Lwu(I { rd, rs1, imm }) => {
            rv64i::emit_lwu(translator, temps, rv(rd), rv(rs1), *imm)
        }
        Instruction::Ld(I { rd, rs1, imm }) => {
            rv64i::emit_ld(translator, temps, rv(rd), rv(rs1), *imm)
        }
        Instruction::Sh(S { rs1, rs2, imm }) => {
            rv64i::emit_sh(translator, temps, rv(rs1), rv(rs2), *imm)
        }
        Instruction::Sw(S { rs1, rs2, imm }) => {
            rv64i::emit_sw(translator, temps, rv(rs1), rv(rs2), *imm)
        }
        Instruction::And(R { rd, rs1, rs2 }) => {
            rv64i::emit_and(translator, temps, rv(rd), rv(rs1), rv(rs2))
        }
        Instruction::Xor(R { rd, rs1, rs2 }) => {
            rv64i::emit_xor(translator, temps, rv(rd), rv(rs1), rv(rs2))
        }
        Instruction::Ori(I { rd, rs1, imm }) => {
            rv64i::emit_ori(translator, temps, rv(rd), rv(rs1), *imm)
        }
        Instruction::Xori(I { rd, rs1, imm }) => {
            rv64i::emit_xori(translator, temps, rv(rd), rv(rs1), *imm)
        }
        Instruction::Srl(R { rd, rs1, rs2 }) => {
            rv64i::emit_srl(translator, temps, rv(rd), rv(rs1), rv(rs2))
        }
        Instruction::Sra(R { rd, rs1, rs2 }) => {
            rv64i::emit_sra(translator, temps, rv(rd), rv(rs1), rv(rs2))
        }
        ...
        Instruction::Sraw(R { rd, rs1, rs2 }) => {
            rv64w::emit_sraw(translator, temps, rv(rd), rv(rs1), rv(rs2))
        }
        ...
        Instruction::Slt(R { rd, rs1, rs2 }) => {
            rv64i::emit_slt(translator, temps, rv(rd), rv(rs1), rv(rs2))
        }
        ...
        Instruction::LrD(R { rd, rs1, rs2 }) => {
            rv64a::emit_lrd(translator, temps, rv(rd), rv(rs1), rv(rs2))
        }
        Instruction::ScW(R { rd, rs1, rs2 }) => {
            rv64a::emit_scw(translator, temps, rv(rd), rv(rs1), rv(rs2))
        }
        Instruction::ScD(R { rd, rs1, rs2 }) => {
            rv64a::emit_scd(translator, temps, rv(rd), rv(rs1), rv(rs2))
        }
